...
class OperatorOracle(BaseOracle):
    ...
    def f(self, z: Point) -> float:
        ...
        result = (
            torch.sum(self._C[None, :, :] * x)
            + 2 * self._C_norm * torch.sum(u * (x.sum(dim=2) - p[None, :]) + v * (x.sum(dim=1) - q))
            + 0.5 * self._gamma * (torch.sum(p * p) + torch.sum(x * x))
        )
    ...
    def grad_x(self, z: Point) -> torch.Tensor:
        ...
        grad = self._C.repeat(denominator, 1, 1) + 2 * self._C_norm * (u[:, :, None] + v[:, None, :]) + self._gamma * x
    ...
    def grad_p(self, z: Point) -> torch.Tensor:
        ...
        return (-u.sum(dim=0) + self._gamma * p) / denominator
